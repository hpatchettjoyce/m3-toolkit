...
- **SVG assets are embedded by `dextrous/build_svg_sprite.py`, not pasted.** It turns the dice icons (`assets/inline icons/dice/`) and the footer's Maleperduis logo into `<symbol>`s between the `SPRITE_START`/`SPRITE_END` markers, prefixing every internal id (the Affinity exports all reuse `_clip1`) and dropping the logo's page-wide `<style>`. Re-run it after changing an asset. Dice tokens in rules text render as icons only in the on-screen caption (`formatRulesText(text, { icons: true })`); `DICE_ICON_STYLE` picks Solid or Void. The header logo is the older hand-inlined `#m3-logo` sprite. Only `setFaviconUrl()` needs a real URL.
