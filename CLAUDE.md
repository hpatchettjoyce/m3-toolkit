...
- **Images in the page are data URIs** (the footer's Maleperduis logo) for the same no-static-hosting reason as the SVG sprite. That is fine inside the page; it is only `setFaviconUrl()` that needs a real URL.
